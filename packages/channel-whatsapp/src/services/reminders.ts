/**
 * Meeting reminder service for WhatsApp
 *
 * Checks upcoming calendar events and sends WhatsApp alerts
...
import { getOwnerJid } from '../security.js';
...
export async function checkAndNotify(): Promise<ReminderCheck> {
  ...
  if (!jid) return { events: [], notified: 0 };
  ...
  for (const event of upcoming) {
    ...
    try {
      ...
      console.warn('[reminders] Failed to send:', error.message);
    }
  }
...
function formatReminder(event: UpcomingEvent): string {
  ...
  if (event.minutesUntil <= 1) {
    return `*Starting now:* ${event.summary} (${time})`;
  }
  return `*In ${event.minutesUntil} min:* ${event.summary} (${time})`;
}
